...
- **`.shop`** → removed from the default grid (owner's decision, 2026-09-16), because it is not a latency problem and there is nothing free left to ask. Measured: `rdap.gmoregistry.net` answers **429 in ~90 ms after 3-4 requests from one IP** and recovers after ~5 s of quiet, so from a shared edge egress IP most `.shop` lookups get no RDAP answer at all. `rdap.org` does not help (it 302s to the same host, from the same IP), and GMO retired port-43 WHOIS on 2026-05-01. The pipeline backs off a throttling registry for 5 s and gives one name one retry when the caller's budget allows; what it will not do is buy a verdict because our own probe was refused.

  **What that means in the product** (`ON_REQUEST_TLDS` in `src/lib/searchableTlds.ts`): the default search is **51 extensions**, `.shop` is not one of them, and a name typed as `acme.shop` still gets a card — an honest *Couldn't verify — this registry limits our checks*, with no Retry button (retrying hits the same wall) and no paid call. `uncertainReason: "registry_throttled"` carries that state through the API. `/tld/shop` stays as a price page: it says DigMyName does not check availability there, names the real reason rather than `.gg`/`.so`'s missing-RDAP one, hides the "Check a .shop name" form, and is `noindex` like every non-searchable extension's page. Re-add it to `SEARCHABLE_TLDS` the day the registry stops throttling us.

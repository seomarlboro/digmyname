      ...
      <div className="flex items-stretch gap-2.5 rounded-3xl border border-border/60 bg-card p-3 backdrop-blur-lg">
        {filterConfigs.map((f) => (
          <button
            key={f.id}
            onClick={() => toggle(f.id)}
            className={`flex min-w-[140px] items-center justify-between gap-3 rounded-2xl px-4 py-2 text-left whitespace-nowrap transition-all ${f.color} ${openFilter === f.id ? "ring-2 ring-primary/30 scale-[1.02]" : "hover:scale-[1.01]"}`}
          >
            <div>

...
### Changed
- README and package description state what was measured instead of "~170 ms": a first-time check lands in about half a second (under 0.9 s at p95, measured from the US and the EU in September 2026), a repeat within 60 s in ~0.1 s from the edge cache. No code changes.
...
### Changed
- Corrected coverage claims to match reality: 6 registrars and 50 TLDs (were listed as 7 / 52).
- Availability signal names corrected: Fastly Domain Research (not the delisted Domainr/RapidAPI endpoint) and DNS-over-HTTPS across three hedged resolvers (Cloudflare, Google, AdGuard).
- Speed wording made honest: "~370 ms typical" full pipeline instead of an unbacked "median … p95 < 1s".

## [1.2.1] — 2026-08-05

### Added
- Official MCP Registry manifest (`server.json`), `mcpName` namespace (`io.github.Seomarlboro/domain-check-skills-mcp`), and `llms-install.md` for directory submissions.
